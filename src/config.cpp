...
Config::Config(const int argc, const char **argv) {
	...
	parser.addExample("mpirun --bind-to none --map-by node --host node0,localhost ./bin/hpmoon -conf \"config.xml\" -ss 480 -trdb \"db/TRdata.txt\" -trnorm");
	...
	parser.addArg("-g", true, "Number of generations."); // Number of generations
	...
	////////////////////// Number of subpopulations (islands) in this MPI process
	this -> nIslands = this -> nSubpopulations / size;
	this -> nIslands += (this -> nSubpopulations % size > rank) ? 1 : 0;


	////////////////////// Number of individuals in the archipelago
	this -> archipelagoSize = this -> nIslands * this -> subpopulationSize;


	////////////////////// Number of individuals in the archipelago (including parents and children)
	this -> archipelagoFamilySize = this -> archipelagoSize << 1;
